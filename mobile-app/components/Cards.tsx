  ...
  return (
    <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-80 relative'>
      ...
      <View className='flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5 gap-1'>
        ...
        <Text className='text-xs font-rubik-bold text-accent-100'>{rating}</Text>
      </View>
      ...
      <View className='flex flex-col items-start absolute bottom-5 inset-x-5 gap-1'>
        ...
        <View className='flex flex-row items-center justify-between w-full'>
          <Text className='text-xl font-rubik-extrabold text-white'>${new Intl.NumberFormat('en-US').format(price)}</Text>
    ...
    <TouchableOpacity onPress={onPress} className='flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'>
      ...
      <View className='flex flex-col mt-2'>
        ...
        <View className='flex flex-row items-center justify-between mt-2'>
          <Text className='text-base font-rubik-bold text-primary-100'>${new Intl.NumberFormat('en-US').format(price)}</Text>
